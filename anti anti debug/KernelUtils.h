...
HANDLE	GetProcessPID(const USHORT* NameOfProcess)
{
	...
	RtlInitUnicodeString(&ClientName, NameOfProcess);
...
bool	isFromAntiDebugRange(PVOID	function)
{
	...
	driverBase = getDriverBaseAddress(&size, Globals::driverName);

	if (((DWORD64)function < ((DWORD64)driverBase + size)) && ((DWORD64)function > ((DWORD64)driverBase)))
	{
		return true;
	}
	else
...
PEPROCESS	getProcessByProcessId(DWORD		processID)
{

	PLIST_ENTRY	ListEntry = (PLIST_ENTRY)(((DWORD64)PsInitialSystemProcess) + Offsets::processLinksOffset);

	PLIST_ENTRY	currentListEntry = ListEntry->Flink;

	while (currentListEntry != ListEntry && currentListEntry != NULL)
	{
		PEPROCESS	currentProcess = (PEPROCESS)((DWORD64)currentListEntry - Offsets::processLinksOffset);

		DbgPrint("current process ID %i \n", PsGetProcessId(currentProcess));


		if ((DWORD)PsGetProcessId(currentProcess) == processID)
		{
